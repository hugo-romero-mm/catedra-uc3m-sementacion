...
class Dealer:
    def __init__(self,apriori_result: list, verbose = False, createDF = False):
        print("Creating apriori dealer...")
        self.result = apriori_result
        self.good_rules = self.get_good_rules(verbose = verbose, createDF = createDF)
    ...
    def get_good_rules(self, good = 'yes_energia', verbose = False, createDF = False):
        ...
        for res in self.result:
            for ordered_statistic in res.ordered_statistics:
                if ordered_statistic.items_base and len(ordered_statistic.items_add) == 1 and good in ordered_statistic.items_add:
...
def apriori(transactions, min_support=.1,min_confidence=.9,min_lift = 1, verbose = False, createDF = False):
    ...
    return Dealer(result, verbose = verbose, createDF = createDF)
