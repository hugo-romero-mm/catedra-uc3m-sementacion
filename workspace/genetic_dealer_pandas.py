...
from fitness_functions import composed_fitness,lift_fitness, lift_and_fisher_fitness, composed_fitness_length_fitness
import wandb
import os
...
class GeneticDealer:
    ...
    def run(self, num_generations=5, generation_size=200, fitness_function="lift", mutation_probability=.01):
        if not self.generation:
            self.generation = [self.generate_fenotype() for _ in range(generation_size)]
        for i in range(num_generations):
            if self.verbose:
                print(f'Generation {i}/{num_generations-1}')
    ...
    def stats(self, fenotype, fitness_function):
        precedent_df = self.df.copy(deep=True)
        for i, gen in enumerate(fenotype):
            if gen:
                col_name = self.index_column_dict[i]
                value = self.index_column_dict[(i, gen)]
                precedent_df = precedent_df[precedent_df[col_name] == value]

        total_count = self.df.shape[0]
        consequent_df = self.df[self.df[self.consequent] == self.consequent_value]
        intersection_df = precedent_df[precedent_df[self.consequent] == self.consequent_value]

        a = precedent_df.shape[0]  # count of precedent
        b = consequent_df.shape[0]  # count of consequent
        ab = intersection_df.shape[0]  # count of intersection of precedent and consequent

        _ab = b - ab  # count of consequent not in precedent
        a_b = a - ab  # count of precedent not in consequent
        _a_b = total_count - a - _ab  # count of neither precedent nor consequent

        fisher = stats.fisher_exact(table=[[ab, _ab], [a_b, _a_b]], alternative="less")[1]

        confidence = ab / a if a else 0
        lift = confidence / self.consequent_support if self.consequent_support else 0
        precedent_support = a / total_count

        conviction = (1 - self.consequent_support) / (1 - confidence) if confidence != 1 else 10_000
        interestingness = (ab ** 2 * total_count - ab ** 3) / (a * b * total_count) if a * b != 0 else 0
